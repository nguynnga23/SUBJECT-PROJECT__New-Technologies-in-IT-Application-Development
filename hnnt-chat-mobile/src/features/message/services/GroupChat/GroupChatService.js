...
export const fetchMessages = async (chatId, token) => {
    try {
        ...
        console.warn('Error fetching messages:', error.response?.data || error.message);
        throw error;
    }
};
...
let chatData = {
    ...
    messages: [
        { id: '101', sender: '@nganguyen92', name: 'Nga Nguyễn', message: 'Trello có đủ tài liệu nha!', time: '18:55' },
        {
            ...
            sender: '@nganguyen92',
            name: 'Nga Nguyễn',
            message: 'Mn nhớ update task trên Trello nhé!',
            time: '18:56',
        },
...
export function handleLongPressMessage(messageId, messages, setMessages, setReplyingMessage, setModalVisible, token) {
    ...
    let options = [
        ...
        {
            text: '🗑️ Delete',
            onPress: () => handleDeleteMessage(messageId, messages, setMessages),
        },
    ];
...
export async function downloadFile(fileUri, fileName) {
    try {
        ...
        const downloadResumable = FileSystem.createDownloadResumable(fileUri, fileDest, {}, (downloadProgress) => {
